...
class SaleController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if( $data['advance_amount']){
            ...
            $data['paid_amount']= $data['advance_amount'];
        }
        ...
        $net_unit_price = $data['net_unit_price']+$data['discount'];
        ...
        if($data['payment_status'] == 3 || $data['payment_status'] == 4 || ($data['payment_status'] == 2 && $data['pos'] && $data['paid_amount'] > 0)) {
            ...
            $lims_payment_data->amount = $data['advance_amount'];
            $lims_payment_data->change = $data['paying_amount'] - $data['advance_amount'];
    ...
    {   
        ...
        $lims_product_data = Product::whereNotIn('type', ['general'])->where('is_active', true)->get();
    ...
    public function posSale()
    {
        ...
        if($role->hasPermissionTo('sales-add')){
            ...
            if(Auth::user()->role_id > 2 && config('staff_access') == 'own') {
                ...
                $recent_sale = Sale::where([['sale_status', 1],['sale_type', 1]])->orderBy('id', 'desc')->take(10)->get();
                $recent_draft = Sale::where('sale_status', 3)->orderBy('id', 'desc')->take(10)->get();
            }
            if(Auth::user()->role_id > 2 && config('staff_access') == 'own') {
                $recent_order = Sale::where([
                    ['sale_status', 1],
                    ['sale_type', 2],
                    ['user_id', Auth::id()]
                ])->orderBy('id', 'desc')->take(10)->get();
                ...
                $recent_order = Sale::where([['sale_status', 1],['sale_type', 2]])->orderBy('id', 'desc')->take(10)->get();
                $recent_draft = Sale::where('sale_status', 3)->orderBy('id', 'desc')->take(10)->get();
            }
            $lims_coupon_list = Coupon::where('is_active',true)->get();
            $flag = 0;

            return view('sale.pos', compact('all_permission', 'lims_customer_list', 'lims_customer_group_all', 'lims_warehouse_list', 'lims_product_list', 'product_number', 'lims_tax_list', 'lims_biller_list', 'lims_pos_setting_data', 'lims_brand_list', 'lims_category_list', 'recent_sale', 'recent_draft','recent_order', 'lims_coupon_list', 'flag'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if($data['balance_amount']){
            $data['sale_type'] = 1;
            $data['paid_amount'] = $data["advance_amount"]+$data["balance_amount"];
